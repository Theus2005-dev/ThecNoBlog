...
export async function GET(request: Request){
  const session = await getServerSession(authOptions)
    try {
          ...
          const [recomendados] = await conn.query("select p.id, p.titulo, p.email_usuario, p.texto, u.nome from posts p inner join usuarios u on p.email_usuario = u.email")

          if ((result as any[]).length > 0) {
           return new Response(JSON.stringify({ body: result, recomendados}), { status: 200 });
          }
